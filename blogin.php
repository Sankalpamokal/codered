...
header("location: blogin.php");//You will be sent to Login.php for re-login 

} 
$inUsername = $_POST["username"]; // as the method type in the form is "post" we are using $_POST otherwise it would be $_GET[] 
$inPassword = $_POST["password"]; 
$stmt= $conn->prepare("SELECT BUSERNAME, BPASSWORD, BNUMBER, BMOBILE, BNAME, BSTATE, BCITY, BLOCAL, APPROVE FROM BLOODBANK WHERE BUSERNAME = ?"); //Fetching all the records with input credentials
...
$stmt->execute();
$stmt->bind_result($UsernameDB, $PasswordDB, $BnumberDB, $BmobileDB, $BnameDB, $BstateDB, $BcityDB, $BlocalDB, $ApproveDB); // Binding i.e. mapping database results to new variables
...
{
echo "<script type='text/javascript'>alert('Your application is under checking, Wait for approval');
...
<div id="radio" style="padding-bottom: 20px">
        <input type="radio" name="source" value="individual" id="individual" onclick="javascript:window.location.href='login.php'; return false;"  >Individual   
          <input type="radio" name="source" value="BB" id="bb" onclick="javascript:window.location.href='blogin.php'; return false;" checked>Bloodbank / Hospital
...
<br/>
				<div class="flex-col-c">
					...
					<a href="individual.php" class="txt2 alink">
						Sign Up
					</a>
				</div>
			</form>
